...
- `shared` (declarada em `infra/`): conecta as **apps** ao RabbitMQ.
- `default` do projeto: conecta cada app ao **seu** Postgres.

O Postgres de um serviço não entra na `shared`, então um serviço não alcança o banco
de outro nem por acidente. O isolamento entre bancos é topológico, não apenas
convencional.
